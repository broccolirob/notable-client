...
mock.onGet('/api/labels').reply(config => {
	return [200, notesDB.labels];
});

mock.onPost('/api/create-note').reply(request => {
...
mock.onPost('/api/update-note').reply(request => {
	const data = JSON.parse(request.data);
...
mock.onPost('/api/update-labels').reply(request => {
	const data = JSON.parse(request.data);
...
mock.onPost('/api/remove-note').reply(request => {
	const data = JSON.parse(request.data);
